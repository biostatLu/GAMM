...
LRTSimZP <- function(
	...
	Z = NULL, # not include the intercept term// 不用加入截距项1，后面会自动加入
	#Z = Z, # 协变量，不包含截距项
	E = E, # 估计的基因表达值
	G = G, # 该基因的cis-SNPs矩阵
	muxi = NULL,
	...
	log_grid_hi = 8,
	log_grid_lo = -10,
	gridlength  = 100,#200好像仅仅多一点时间，建议200
	lambda0 = 0,
	seed = NA,
	#REML = FALSE,
	parallel = c("no", "multicore", "snow"),#snow不能用;multicore可显著提高速度
	ncpus = 1L,
	cl = NULL) {
